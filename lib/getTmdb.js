...
async function initMovieDb() {
  ...
  initializationPromise = (async () => {
    ...
    moviedb = new MovieDb(process.env.TMDB_API_KEY, "https://api.themoviedb.org/3/", 45); //max 45 requests per second, as default = 50 sometimes results in 429 (official limit is around 50)
    return moviedb;
  })();
...
async function getCollectionIDsFromMovieIDs(movieIds) {
  ...
  return new Set(movieDetails.filter((movie) => movie && movie.belongs_to_collection?.id).map((movie) => movie.belongs_to_collection.id));
}
