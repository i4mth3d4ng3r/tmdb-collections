...
async function getImdbId(movieId) {
  return await moviedb
    ...
    .catch((err) => {
      console.error(`Error getting imdb id for ${movieId}: ${err.response?.status} - ${err.response?.data?.status_message}`);
      return null;
    });
}
...
async function getGenres() {
  try {
    ...
    console.error(`Error getting genres: ${err.response?.status} - ${err.response?.data?.status_message}`);
    return [];
  }
}
...
async function getMovieDetails(movieId) {
  return cacheWrapMovieDetails(movieId, async () => {
    return await moviedb
      ...
      .catch((err) => {
        console.error(`Error getting movie details for ${movieId}: ${err.response?.status} - ${err.response?.data?.status_message}`);
        return null;
      });
  });
}
...
async function getLogo(movieId) {
  try {
    ...
    console.error(`Error getting movie logo for ${movieId}: ${err.response?.status} - ${err.response?.data?.status_message}`);
    return null;
  }
}
