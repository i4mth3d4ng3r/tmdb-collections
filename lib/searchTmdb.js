...
async function searchMoviesByPerson(query) {
  try {
    ...
    const personId = personResponse.results.sort((a, b) => b.popularity - a.popularity)[0].id;
    ...
    return new Set([
      ...creditsResponse.cast.map((movie) => movie.id),
      ...creditsResponse.crew
        .filter((crew) => crew.department === "Writing" || crew.department === "Directing")
        .map((movie) => movie.id),
    ]);
  } catch (err) {
...
async function getSearch(query) {
  try {
    ...
    return [...allCollectionIds].slice(0, 30); // limit for performance...more than 30 collections is needed only if the search is too wide
  } catch (err) {
