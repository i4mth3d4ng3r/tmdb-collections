...
async function getCatalogResponse(req) {
  ...
  const processCollections = async (collections, config) => {
    ...
    for (let i = 0; i < collections.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        // console.error(`Error processing collections batch for ${cacheKey} starting at index ${i}:`, error);
        console.error(`Error processing collections batch for ${cacheKey} starting at index ${i}:`, {
          error: error.message,
          stack: error.stack,
          batch: batch,
          batchIndex: Math.floor(i / BATCH_SIZE) + 1,
          totalBatches: Math.ceil(collections.length / BATCH_SIZE),
        });
      }
    }
  ...
  const metas = (
    cacheLanguages.includes(config.language) && !extra.search
      ? // individual collections are cached so this caching is duplicate, but loading whole catalog from cache and applying configured caching takes 3s in average, which is slow
        // this will cache whole catalog for configured languages, I think it is good compromise
        await cacheWrapCollectionDetails("lang:" + config.language + ", catalog:" + cacheKey, async () => {
          return processCollections(collections, config);
        })
      : await processCollections(collections, config)
  ).filter((collection) => config.enableAdultContent || (!config.enableAdultContent && !collection?.adult));

  const endTime = process.hrtime(startTime);
  const milliseconds = endTime[0] * 1000 + endTime[1] / 1000000;
  console.log(
    `Catalog request for language "${config.language}", type "${req.params.type}", ${cacheKey}\nfetching time: ${milliseconds.toFixed(2)}ms`
  );
