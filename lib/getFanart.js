    ...
    return await cacheWrapCollectionDetails(`${imageType}:${ID}`, async () => {
      ...
      const fanartData = {
        ...
        ...(imageType === "hdmovielogo" && {
          hdmovielogo: data.hdmovielogo.map((logo) => ({ lang: logo.lang === "cz" ? "cs" : logo.lang, url: logo.url, likes: logo.likes })),
        }),
        ...(imageType === "moviethumb" && {
          moviethumb: data.moviethumb.map((thumb) => ({ lang: thumb.lang === "cz" ? "cs" : thumb.lang, url: thumb.url, likes: thumb.likes })),
        }),
      };
