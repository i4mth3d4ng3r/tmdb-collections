...
async function getMetaResponse(req) {
  ...
  try {
    ...
    console.error(`Error in getMetaResponse for language ${config.language} and type ${req.params.type} and id ${collectionId}:`, error);
    return { meta: [] };
  }
  ...
  console.log(
    `Metadata request for language ${config.language} and type ${req.params.type} and id ${collectionId}\nfetching time: ${milliseconds.toFixed(2)}ms`
  );
