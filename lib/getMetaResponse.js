...
async function getMetaResponse(req) {
    ...
    if (config.language !== "en") {
      ...
      collection.videos = collection.videosTmp.map((video) => {
        ...
        return {
          ...
          overview: translation?.description || video.overview,
        };
      });
    } else {
      collection.videos = collection.videosTmp;
    }
        ...
        await Promise.all(
          ...
          collection.videos.map(async (video) => {
            ...
            if (video.id) {
              ...
              thumbnailUrl = await getFanart(video.id, "moviethumb").then((data) => {
  ...
  console.log(
    `Metadata request for lang: "${config.language}", type: "${req.params.type}", id: "${collectionId}"\nfetching time: ${milliseconds.toFixed(2)}ms`
  );
