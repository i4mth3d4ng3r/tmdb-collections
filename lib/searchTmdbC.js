...
async function searchMovies(query, includeAdult = false) {
  try {
    const maxResults = 99;
    const maxPages = 6;
...
async function searchCollections(query, includeAdult = false) {
  try {
    const maxResults = 30;
    const maxPages = 3;
...
async function searchMoviesByPerson(query, includeAdult = false) {
  try {
...
async function getSearch(query, config = {}) {
  try {
    const includeAdult = config.enableAdultContent === true;

    // Run all searches in parallel
    const [directCollectionIds, personMovieIds] = await Promise.all([
      searchCollections(query, includeAdult),
      searchMoviesByPerson(query, includeAdult),
    ]);
    ...
    if (allCollectionIds.size === 0) {
      const movieIds = await searchMovies(query, includeAdult);
      if (movieIds.size > 0) {
