    ...
    if (totalPages > 1) {
      ...
      for (let page = 2; page <= totalPages; page++) {
        try {
          ...
          console.error(`Movie search page ${page}: ${err.response?.status}`);
...
async function searchCollections(query) {
  try {
    ...
    if (totalPages > 1) {
      ...
      for (let page = 2; page <= totalPages; page++) {
        try {
          const response = await moviedb.searchCollection({ query, page });
          if (response.results && response.results.length > 0) {
          ...
          console.error(`Collection search page ${page}: ${err.response?.status}`);
          break; // Stop on first error
        }
      }
    }
    ...
    console.error(`Error during collection search: ${err.response?.status} - ${err.response?.data?.status_message}`);
    return new Set();
  }
}
