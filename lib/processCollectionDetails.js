...
async function processCollectionDetails(collectionId) {
  return cacheWrapCollectionDetails(collectionId, async () => {
    ...
    if (!collectionDetails || !firstMovieDetails || !imdbIds) {
      return null;
    }
    ...
    return {
      ...
      latestReleaseDate: new Date(releasedParts[0]?.release_date || 0), // for collections sorting
      ...
      poster: `https://image.tmdb.org/t/p/w500${
        collectionDetails.poster_path || parts[parts.length - 1].poster_path || parts[parts.length - 2].poster_path
      }`,
      background: `https://image.tmdb.org/t/p/original${
        collectionDetails.backdrop_path || parts[parts.length - 1].backdrop_path || parts[parts.length - 2].backdrop_path
      }`,
