...
async function processCollectionDetails(collectionId, config) {
  ...
  collection.genres = collection.genres.map((genre) => {
    try {
      ...
      if (!genreObj) {
        throw new Error(`Genre with id ${genre} not found`, { genres: manifest.genres });
      }
      return genreObj.name;
    } catch (error) {
      console.error(`Error mapping genre ${genre}:`, error.message);
      return "Unknown"; // Fallback value in case of error
    }
  });
