...
async function processCollectionDetails(collectionId) {
  return cacheWrapCollectionDetails(collectionId, async () => {
    ...
    return {
      ...
      releaseInfo: `${new Date(parts[0].release_date).getFullYear() || new Date(parts[1].release_date).getFullYear()}-${
        new Date(parts[parts.length - 1].release_date).getFullYear() + " (New part soon)"
      }`,
      ...
      videos: parts.map((movie, index) => ({
        id: imdbIds[index],
        title: movie.title,
        released: movie.release_date ? movie.release_date + "T00:00:00.000Z" : null,
        season: 1,
        episode: index + 1,
