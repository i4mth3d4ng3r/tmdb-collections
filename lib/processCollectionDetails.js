...
async function processCollectionDetails(collectionId, config) {
  ...
  // Batch process video details instead of making separate async calls for each video
  const videosByTmdbId = {};
  const movieDetailsPromises = [];

  collection.videos.forEach((video) => {
    videosByTmdbId[video.tmdbId] = video;
    movieDetailsPromises.push(getMovieDetails(video.tmdbId)); // Should be already cached
  });

  const movieDetailsResults = await Promise.all(movieDetailsPromises);

  // Apply translations in a single loop
  movieDetailsResults.forEach((movieDetails) => {
    if (!movieDetails) return;

    const video = videosByTmdbId[movieDetails.id];
    if (!video) return;

    const translation = movieDetails.translations.find((t) => t.iso_639_1 === config.language);
    if (translation) {
      video.title = translation.title || video.title;
      video.overview = translation.description || video.overview;
    }

    // Find best poster in the requested language
    const localizedPosters = movieDetails.posters.filter((p) => p.iso_639_1 === config.language);
    if (localizedPosters.length > 0) {
      video.poster = localizedPosters.sort((a, b) => b.vote_average - a.vote_average)[0].poster || video.poster;
    }
  });
