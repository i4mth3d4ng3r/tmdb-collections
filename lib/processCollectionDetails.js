...
async function processCollectionDetails(collectionId, config) {
  ...
  const collection = structuredClone(
    //dont want to modify the cached collection in the language mapping part below
    await cacheWrapCollectionDetails(collectionId, async () => {
      ...
      testCaching = [...new Set(releasedParts.reverse().flatMap((movie) => movie.genre_ids || []))];
      console.log(`CollectionId caching: ${collectionId}`, {
      ...
      return {
        ...
        genres: testCaching, //get all genres from all movies
  ...
  console.log(`CollectionId returned from cache: ${collectionId}`, {
    configLang: config.language,
  ...
  collection.genres = collection.genres
    .map((genreId) => {
      try {
        const genreObj = manifest.genres.find((g) => g.id == genreId);
        if (!genreObj) {
          throw new Error(`CollectionId: ${collectionId}, Genre with id ${genreId} not found`);
        }
        ...
        // Return null for genres that can't be mapped, and filter them out later
        return null;
      }
    })
  ...
  delete collection.translations, collection.posters;
