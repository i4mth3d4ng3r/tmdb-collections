...
async function processCollectionDetails(collectionId, config) {
  ...
  const collection = await cacheWrapCollectionDetails(collectionId, async () => {
    ...
    return {
      ...
      genres: [...new Set(moviesDetails.reverse().flatMap((movie) => movie?.genres.map((genre) => genre.id) || []))], //get all genres from all movies
  ...
  collection.genres = collection.genres.map((genre) => {
    try {
      const genreObj = manifest.genres.find((g) => g.id == genre);
      if (!genreObj) {
        throw new Error(`CollectionId: ${collectionId}, Genre with id ${genre} not found`);
      }
      return genreObj.name;
    } catch (error) {
      console.error(error.message, { configLang: config.language, genres: manifest.genres, collectionGenres: collection.genres });
    }
  });
