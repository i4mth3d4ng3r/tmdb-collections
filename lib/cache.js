...
function updateMetrics(key, value, isHit, isRefresh = false) {
  ...
  if (isLruKey) {
    ...
    if (lruKeys.length >= lruCacheSize) {
      ...
      // We'll need to get the actual value from cache to calculate proper size
      // For now, we'll use an estimate based on average key size
      const oldPrefix = getKeyPrefix(oldestLruKey);

      // Just reduce the total and prefix counters by a reasonable estimate
      // This is safer than using the wrong value's size
      if (metrics.sizes[oldPrefix] > 0) {
        const avgKeySize = metrics.sizes[oldPrefix] / lruKeys.filter((k) => getKeyPrefix(k) === oldPrefix).length;
        const estimatedSize = Math.min(avgKeySize, metrics.sizes[oldPrefix]); // Don't subtract more than what's available

        if (estimatedSize > 0) {
          metrics.sizes.total = Math.max(0, metrics.sizes.total - estimatedSize);
          metrics.sizes[oldPrefix] = Math.max(0, metrics.sizes[oldPrefix] - estimatedSize);
        }
      }
    ...
    if (nonLruKeys.length >= remainingSpace) {
      ...
      // Similar to LRU keys, use an estimate
      const oldPrefix = getKeyPrefix(oldestNonLruKey);

      if (metrics.sizes[oldPrefix] > 0) {
        const prefixKeys = nonLruKeys.filter((k) => getKeyPrefix(k) === oldPrefix);
        const avgKeySize = prefixKeys.length ? metrics.sizes[oldPrefix] / prefixKeys.length : 0;
        const estimatedSize = Math.min(avgKeySize, metrics.sizes[oldPrefix]);
  ...
  if ((!isHit || isRefresh) && value !== undefined) {
    ...
    if (size > 0) {
      // For refreshes, don't subtract - the old value is no longer accessible
      // Just add the new size directly
...
// Completely recalculate metrics from scratch
async function recalculateMetrics() {
  console.log("[CACHE] Starting full metrics recalculation");

  // Reset all metrics
  resetMetrics();

  // Get all keys we're tracking
  const allKeys = Array.from(metrics.keys);
  metrics.keys.clear();
  ...
  for (let i = 0; i < allKeys.length; i += batchSize) {
    ...
    console.log(`[CACHE] Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(allKeys.length / batchSize)}`);
    ...
    for (const key of batch) {
      ...
      try {
        if (isLruKey) {
          value = await lruCache.get(key);
        } else {
          value = await ttlCache.get(key);
        }
      } catch (err) {
        console.error(`[CACHE] Error fetching value for ${key}:`, err.message);
        continue;
      }
      ...
      if (value !== undefined && value !== null) {
        // Add to metrics
        metrics.keys.add(key);
        metrics.lastAccessed[key] = Date.now(); // We don't know actual last access time
  ...
  console.log("[CACHE] Metrics recalculation completed");
  console.log(
    "[CACHE] Current metrics:",
    Object.entries(metrics.sizes)
      .map(([key, size]) => `${key}: ${formatSize(size)}`)
      .join(", ")
  );

  return getCachePerformance();
}
