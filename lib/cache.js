...
const CATALOG_TTL = 2.5 * 24 * 60 * 60 * 1000; // 2.5 days - with 1.5 days refresh threshold it will be 1 full day of caching
const COLLECTION_TTL = 7 * 24 * 60 * 60 * 1000; // 7 days
const SEARCH_TTL = CATALOG_TTL / 2.5; // 1 day - should be similar to catalog ttl
const MOVIE_TTL = 30 * 24 * 60 * 60 * 1000; // 30 days - some big number to treat possible issues with lru cache
const CATALOG_REFRESH_THRESHOLD = 1.5 * 24 * 60 * 60 * 1000; // 1.5 days windows in which some cache call executes refresh
const MAX_KEYS_TO_TRACK = 50000; // Limit number of keys we track

let lruCache = null;
const lruCacheSize = 10000; //max number of keys in lru cache
let ttlCache = null;
