...
const CATALOG_TTL = 2.4 * 24 * 60 * 60 * 1000; // 2.4 days - with 1.5 days refresh threshold it will be cached for 0.9 of the day (buffering cron runs every 1 full day, so catalogs will be refreshed every day)
const CATALOG_REFRESH_THRESHOLD = 1.5 * 24 * 60 * 60 * 1000; // 1.5 days windows in which some catalog call (e. g. buffering cron) executes refresh
const COLLECTION_TTL = 0.9 * 24 * 60 * 60 * 1000; // 0.9 days - should be similar to catalog cache time
...
let lruCache = null;
const lruCacheSize = 2000; //max number of keys in lru cache
let ttlCache = null;
...
const metrics = {
  ...
  sizes: {
    total: 0,
    [CATALOG_KEY_PREFIX]: 0,
    [COLLECTION_KEY_PREFIX]: 0,
    [MOVIE_KEY_PREFIX]: 0,
    [SEARCH_KEY_PREFIX]: 0,
  },
  typeStats: {
    [CATALOG_KEY_PREFIX]: { hits: 0, misses: 0 },
...
// Catalog-specific cache wrapper with refresh threshold
async function cacheWrapCatalog(id, method, options = {}) {
  const { ttl, refreshThreshold, cache } = options;
  return cacheWrap(`${CATALOG_KEY_PREFIX}:${id}`, method, {
    ttl: ttl || CATALOG_TTL,
    refreshThreshold: refreshThreshold || CATALOG_REFRESH_THRESHOLD,
    cache: cache || ttlCache,
  });
}

// Collection-specific cache wrapper
